...
std::vector<JointCommand> HqpWbc::getCommandMessage(scalar_t currentTime, const vector_t &currentState,
                                                        ...
                                                        int & flag) 
{
    ...
    if (nContacts_ == 4) {
        // QP constraints
        Task task1 = createDynamicsTask() + createContactForceTask() + createTorqueLimitTask();
        if (stanceAsConstraint_) {
            task1 = task1 + createStanceFootNoMotionTask();  // additional constraints for stance
        }
        ...
        hqpSolution = hqpSolver_.solveHqp(tasks, flag);
    } else {
        // QP constraints
        Task task1 = createDynamicsTask() + createContactForceTask() + createTorqueLimitTask();
        if (stanceAsConstraint_) {
...
void HqpWbc::loadSettings(const std::string &configFile) {
    ...
    hqpSolver_.maxRecalculations_ = maxRecalculations_;
}
