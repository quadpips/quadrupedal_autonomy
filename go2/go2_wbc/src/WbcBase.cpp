...
WbcBase::WbcBase(const std::string &configFile, const std::string &urdfString,
    ...
    : pinocchioInterfaceMeasured_(go2::createQuadrupedPinocchioInterfaceFromUrdfString(urdfString)),
      ...
      kinematicsPtr_(kinematics.clone()) 
{
    ...
    // BE CAREFUL TO MAKE SURE THESE AGREE WITH URDF MODLE
    footNames_ = {"FL_foot", "FR_foot", "RL_foot", "RR_foot"};
...
Task WbcBase::createContactForceTask() 
{
    ...
    for (size_t i = 0; i < NUM_CONTACT_POINTS; ++i) 
    {
        if (contactFlags_[i]) 
        {
            // // get closest region normal
            // vector3_t normal = switchedModelPreComp.getSurfaceNormalInOriginFrame(leg);

            // // re-calculated muMatrix_ for each contact point
...
Task WbcBase::createTorqueLimitTask() 
{
    ...
    // upper bound
    f.head<JOINT_COORDINATE_SIZE>() = torqueLimits_;
    f.head<JOINT_COORDINATE_SIZE>() -= hj;

    // lower bound
    f.tail<JOINT_COORDINATE_SIZE>() = torqueLimits_;
...
{
    // std::cout << "[createBaseAccelerationTask]" << std::endl;
    ...
    auto linVelocityError = stateDesired.segment<3>(9) - stateCurrent.segment<3>(9);

    /* Base orientation error*/
    vector3_t eulerCurrent = stateCurrent.head<3>();
    vector3_t eulerDesired = stateDesired.head<3>();
    vector3_t orientationError = rotationErrorInLocalEulerXYZ(eulerCurrent, eulerDesired);

    // std::cout << "  eulerCurrent: " << eulerCurrent.transpose() << std::endl;
    // std::cout << "  eulerDesired: " << eulerDesired.transpose() << std::endl;
    // std::cout << "  orientationError: " << orientationError.transpose() << std::endl;

    /* Base angular velocity error */
    vector3_t w_current = stateCurrent.segment<3>(6);
    vector3_t w_desired = stateDesired.segment<3>(6);
    vector3_t angVelocityError = w_desired - w_current;
    ...
    // std::cout << "  baseAccelerationLocal (lin): " << baseAccelerationLocal.tail<3>().transpose() << std::endl;
    // std::cout << "  positionError: " << positionError.transpose() << std::endl;
    // std::cout << "  orientationError: " << orientationError.transpose() << std::endl;

    // std::cout << "  baseAccelerationLocal (ang): " << baseAccelerationLocal.head<3>().transpose() << std::endl;
    // std::cout << "  linVelocityError: " << linVelocityError.transpose() << std::endl;
    // std::cout << "  angVelocityError: " << angVelocityError.transpose() << std::endl;

    // desired base linear acceleration (expressed in base frame)
    baseAcceleration.head<3>() = baseAccelerationLocal.tail<3>() + baseKp_ * positionError + baseKd_ * linVelocityError;

    // desired base angular acceleration (expressed in base frame)
    baseAcceleration.tail<3>() = baseAccelerationLocal.head<3>() + eulerKp_ * orientationError + eulerKd_ * angVelocityError;
...
{
    // std::cout << "[createSwingFootAccelerationTask]" << std::endl;
    ...
    const size_t Arows = 3 * (stanceAsConstraint_ ? NUM_CONTACT_POINTS - nContacts_
                                                  : NUM_CONTACT_POINTS);
    matrix_t A = matrix_t::Zero(Arows, nDecisionVariables_);
    vector_t b = vector_t::Zero(Arows);
    ...
    for (size_t i = 0; i < NUM_CONTACT_POINTS; ++i) 
    {
        ...
        {
            // std::cout << "  swing foot " << footNames_[i] << std::endl;
            ...
            const vector_t posError = posDesired - posMeasured;
            const vector_t velError = velDesired - velMeasured;

            // std::cout << "    posDesired: " << posDesired.transpose() << std::endl;
            // std::cout << "    posMeasured: " << posMeasured.transpose() << std::endl;
            // std::cout << "    posError: " << posError.transpose() << std::endl;
            // std::cout << "    velDesired: " << velDesired.transpose() << std::endl;
            // std::cout << "    velMeasured: " << velMeasured.transpose() << std::endl;
            // std::cout << "    velError: " << velError.transpose() << std::endl;
            
            const vector_t footAcceleration = swingKp_ * posError + swingKd_ * velError;
...
{
    // std::cout << "[createContactForceMinimizationTask]" << std::endl;
...
void WbcBase::loadSettings(const std::string &configFile, const std::string &configPrefix) 
{
    ...
    using ocs2::loadData::loadEigenMatrix;
    ...
    // vector_t tempTorqueLimits(3);
    loadEigenMatrix(configFile, configPrefix + "torqueLimits", torqueLimits_);
    // torqueLimits_ = tempTorqueLimits;

    // std::cout << "torqueLimits_: " << torqueLimits_.transpose() << std::endl;
