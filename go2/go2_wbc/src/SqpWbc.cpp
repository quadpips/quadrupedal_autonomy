...
std::vector<JointCommand> SqpWbc::getCommandMessage(scalar_t currentTime, const vector_t &currentState,
                                                        ...
                                                        int & flag) 
{
    ...
    // QP cost function
    Task baseAccelerationTask = createBaseAccelerationTask(currentState, desiredState, desiredInput, desiredJointAcceleration);
    Task contactForceTask = createContactForceMinimizationTask(desiredInput);
    Task swingFootAccelerationTask = createSwingFootAccelerationTask(currentState, currentInput, desiredState, desiredInput);
    Task weightedTasks =    weightBaseAcceleration_ * baseAccelerationTask +
                            weightContactForce_ * contactForceTask +
                            weightSwingLeg_ * swingFootAccelerationTask;

    // solve QP
    vector_t sqpSolution = sqpSolver_.solveSqp(weightedTasks, constraints, flag);
...
void SqpWbc::loadSettings(const std::string &configFile) 
{
    ...
    loadCppDataType<scalar_t>(configFile, prefix + "maxRecalculations", maxRecalculations_);
    sqpSolver_.maxRecalculations_ = maxRecalculations_;
}
